/**
 * Orbit — Firestore Mock Data Seeding Script
 * Seeds 30 providers across Karachi & Islamabad
 * Run: node seed.js
 */
const admin = require('firebase-admin');
const serviceAccount = require('./service-account.json'); // Add your service account key
...
function slots(baseHour) {
  return [
    admin.firestore.Timestamp.fromDate(new Date(tomorrow.setHours(baseHour, 0, 0, 0))),
    admin.firestore.Timestamp.fromDate(new Date(tomorrow.setHours(baseHour + 2, 0, 0, 0))),
    admin.firestore.Timestamp.fromDate(new Date(dayAfter.setHours(baseHour, 0, 0, 0))),
  ];
}
...
const providers = [
  // ── AC TECHNICIANS ──────────────────────────────────────────────────────────
  {
    ...
    location: new admin.firestore.GeoPoint(33.6982, 72.9975), city: 'Islamabad', area: 'G-13',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(24.8107, 67.0655), city: 'Karachi', area: 'DHA Karachi',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(24.9295, 67.0970), city: 'Karachi', area: 'Gulshan',
    // STRESS TEST: High rating but unreliable — must be demoted
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(33.7150, 73.0170), city: 'Islamabad', area: 'F-10',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(24.8607, 67.0104), city: 'Karachi', area: 'Saddar',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(33.5498, 73.1946), city: 'Islamabad', area: 'Bahria Town',
  ...
  // ── PLUMBERS ─────────────────────────────────────────────────────────────────
  {
    ...
    location: new admin.firestore.GeoPoint(24.9478, 67.0529), city: 'Karachi', area: 'North Nazimabad',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(24.8117, 67.0300), city: 'Karachi', area: 'Clifton',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(33.6823, 73.0594), city: 'Islamabad', area: 'I-8',
  ...
  },
  {
    ...
    location: new admin.firestore.GeoPoint(33.7251, 73.0062), city: 'Islamabad', area: 'E-11',
...
async function seed() {
  ...
  for (const provider of providers) {
    const ref = db.collection('providers').doc(provider.uid);
    batch.set(ref, { ...provider, createdAt: now });
  }
  await batch.commit();
  console.log(`✅ Seeded ${providers.length} providers successfully!`);
  console.log('📊 Distribution: 6 AC, 5 Plumber, 5 Electrician, 4 Beautician, 4 Tutor, 3 Mechanic, 3 Home Service');
  console.log('⚠️  Stress test provider: ac_003 (Ali Hassan) — high rating 4.7 but 22% cancellation rate → should be demoted by AI');
  process.exit(0);
}
